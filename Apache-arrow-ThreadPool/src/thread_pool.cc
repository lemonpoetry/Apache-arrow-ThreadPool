...
Status ThreadPool::SetCapacity(int threads) {
  ...
  // pending_tasks 代表的是 任务队列  workers代表的是当前的可工作线程数
...
// 通过环境变量灵活地控制线程池的行为，同时也提供了一种默认的行为以应对未设置环境变量的情况
int ThreadPool::DefaultCapacity() {
  int capacity, limit;
  capacity = ParseOMPEnvVar("OMP_NUM_THREADS");
  if (capacity == 0) {
    // 如果该环境变量未设置或其值为0，则使用hardware_concurrency()函数来获取硬件支持的并发线程数
    capacity = std::thread::hardware_concurrency();
  }

  // 这意味着，如果环境变量OMP_THREAD_LIMIT设置了一个具体的线程数上限，那么实际使用的线程数将不超过这个上限
  limit = ParseOMPEnvVar("OMP_THREAD_LIMIT");
  if (limit > 0) {
    capacity = std::min(limit, capacity);
  }
  // limit=0 即代表未设置该值，此时设置默认值capacity=4
  if (capacity == 0) {
